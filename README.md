...
- 独立的极简无标题悬浮窗：显示对应游戏技能图标、下一技能名称与剩余秒数
- 悬浮窗可拖动、锁定、自定义宽高、真实像素字号、透明度并启用鼠标穿透；字号变化会重新生成字体，不使用模糊的窗口缩放
- 原有完整时间轴窗口与计划编辑器继续保留
- 当前/下一技能大字提醒、倒计时和未来项目表格
- 优先通过 EdgeTTS.Dalamud IPC 播报，未安装或未就绪时回退 Windows TTS；Dalamud 通知可独立开关
...
可用参数：`overlay`（开关悬浮窗）、`unlock`（解除锁定与鼠标穿透）、`config`、`start`、`reset`、`pause`、`test`、`+1`、`-1`。

从主窗口、设置页或命令手动打开悬浮窗时，会覆盖“仅运行时显示”和“仅匹配副本职业”过滤条件，直到再次手动关闭。设置页会显示当前窗口状态或自动隐藏原因。
